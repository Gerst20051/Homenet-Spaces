...
case 'inbox':
$query = 'SELECT * FROM ' . mysql_real_escape_string($_SESSION['username'], $comment_db) . ' WHERE type = 0 ORDER BY date DESC';
break;

case 'sent':
$query = 'SELECT * FROM ' . mysql_real_escape_string($_SESSION['username'], $comment_db) . ' WHERE type = 1 ORDER BY date DESC';
break;
}
...
?>: <?php echo '<a href="user_profile.php?id=' . $row2['user_id'] . '">' . $row2['user'] . '</a>'; if (($row2['status'] == 0) && ($row2['type'] == 1)) { echo ' | <span style="color : #ff0000; "><small>Unread!</small></span>'; } ?>
</div>
<div style="clear : right; float : right; text-align : right; padding : 10px; width : 45%; ">
<?php if (($row2['status'] == 0) && ($row2['type'] == 0)) { echo '<span style="color : #ff0000; font-weight : bold; ">New!</span> | '; } echo $row2['date']; ?>
</div>
</div>
<div style="background-color : #ffffff; clear : both; display : block; margin : 0 auto; margin-left : 10px; margin-right : 10px; min-height : 50px; padding : 10px; padding-bottom : 16px;  text-align : left; ">
<div style="display : block; text-align : center; width : 10%; ">
<?php
if ($row3['default_image'] != null) {
echo '<img src="uploads/' . $row2['user'] . '/images/thumb/' . $row3['default_image'] . '" id="defaultuserimage" title="' . $firstname . ' ' . $lastname . '" style="float : left; height : 50px; margin-right : 10px; margin-top : 4px; width : 50px; " />' . "\n";
} else {
...
$query = 'UPDATE ' . $_SESSION['username'] . ' SET
status = 1
WHERE
...
$query = 'UPDATE ' . $row2['user'] . ' SET
status = 1
WHERE
comment_id = ' . $row2['user_cid'] . '
LIMIT 1';
mysql_query($query, $comment_db) or die(mysql_error());
}
}
