...
case 'inbox':
$query = 'SELECT * FROM ' . mysql_real_escape_string($_SESSION['username'], $message_db) . ' WHERE type = 0 ORDER BY date DESC';
break;

case 'sent':
$query = 'SELECT * FROM ' . mysql_real_escape_string($_SESSION['username'], $message_db) . ' WHERE type = 1 ORDER BY date DESC';
break;
}
...
echo '</small></a>'; if (($row2['status'] == 0) && ($row2['type'] == 1)) { echo ' | <span style="color : #ff0000; "><small>Unread!</small></span>'; } ?>
</div>
...
echo '<img src="uploads/' . $row2['user'] . '/images/thumb/' . $row3['default_image'] . '" id="defaultuserimage" title="' . $fullname . '" style="float : left; height : 50px; margin-right : 10px; margin-top : 4px; width : 50px; " />' . "\n";
} else {
